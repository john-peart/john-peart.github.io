...
{
  _hlContainer = document.getElementById(containerName);
  _hlConfig = generateHLConfig(data.pokemon,data.maxStats);
  _hyperList = HyperList.create(_hlContainer, _hlConfig);

  window.onresize = e => {
    _hlConfig.height = window.innerHeight;
    _hyperList.refresh(_hlContainer, _hlConfig);
  };  
}

function generateHLConfig(pokemonData,maxStats)
{
  return {
    ...
    generate(row) {      
      ...
      el.innerHTML = GeneratePokeCardHtml(pokemonData[row],maxStats);
      return el;
    }
  };
}
